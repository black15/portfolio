...
const Header = (props: Props) => {
  ...
  return (
    <div>
      <header
        className={`relative hidden lg:flex flex-row items-center justify-between px-6 py-4 bg-transparent`}
      >
        <Link
          href={
            "https://drive.google.com/file/d/16QonyRZm9nYi-wZfICDimCzLrc7WnYvg/view?usp=sharing"
          }
        >
          <h1 className={`text-4xl font-semibold flex items-center`}>
            RESUME{" "}
            <HiDocumentDownload className="transition hover:text-[#00e064] cursor-pointer" />
          </h1>
        </Link>
        <div>
          <ul className="flex items-center space-x-4 font-medium uppercase">
            <li className=" transition hover:text-[#00e064]">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex flex-row items-center space-x-4">
            <Link href={"https://facebook.com/unknownkid.18"} target="_blank">
              <FaFacebookF fontSize={22} />
            </Link>
            <Link href={"https://twitter.com/oussama1337"} target="_blank">
              <BsTwitter fontSize={22} />
            </Link>
            <Link href={"https://github.com/black15"} target="_blank">
              <BsGithub fontSize={22} />
            </Link>
            <Link
              href={"https://instagram.com/o.u.s.s._.a.m.a"}
              target="_blank"
            >
      ...
      <div className="fixed left-0 right-0 mx-auto w-[95%] sm:w-5/6 md:w-2/3 bottom-5 lg:hidden transition z-30 rounded-full bg-slate-300 dark:bg-slate-800">
        <div className="flex flex-row justify-evenly items-center px-12 py-2">
          <Link
            href={"#about"}
            className="flex flex-col space-y-2 items-center bg-slate-200 dark:bg-slate-900 px-4 py-2 rounded-full h-16 w-16"
          >
            <SiAboutdotme fontSize={22} className="text-[#00e064]" />
            <small className="text-[10px] font-poppins font-semibold">
              ABOUT
            </small>
          </Link>
